...
exports.register = [
	...
	body("password").isLength({ min: 6 }).trim().withMessage("Password must be 6 characters or longer."),
	// Sanitize fields.
...
exports.login = [
  body("email").isLength({ min: 1 }).trim().withMessage("Email cannot be empty")
	...
	body("password").isLength({ min: 1 }).trim().withMessage("Password cannot be empty"),
	sanitizeBody("email").escape(),
	sanitizeBody("password").escape(),
	(req, res) => {
		console.log(req);
